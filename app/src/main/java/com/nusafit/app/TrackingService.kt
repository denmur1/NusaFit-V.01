...
class TrackingService : Service() {
    ...
    private fun stopTracking() {
        ...
        val prefs = getSharedPreferences("nusafit", MODE_PRIVATE)
        val media = prefs.getString("pending_media", "").orEmpty()
            .split("|")
            .filter { it.isNotBlank() }
        val a = ActivityRecord(
            java.util.UUID.randomUUID().toString(),
            name, type, start, end, distance, kcal, fat, points.toList(), media
        )
        Store.addActivity(this, a)
        if (media.isNotEmpty()) Store.addMedia(this, a.id, media)
        prefs.edit()
            .putBoolean("tracking_active", false)
            .remove("pending_media")
            .remove("pending_name")
            .remove("pending_type")
            .apply()
