...
class MainActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    private fun clearContent() {
        // Remove any MapFragment before its container disappears. Keeping a fragment
        // attached to a removed dynamic container can leave a stale map instance
        // and cause rendering/transaction problems after repeated navigation.
        supportFragmentManager.findFragmentByTag("NusaFitMap")?.let { fragment ->
    ...
    private fun showHome() {
        ...
        grid.addView(tool("📷  Foto / Video Aktivitas") { mediaPicker.launch(arrayOf("image/*", "video/*")) }, LinearLayout.LayoutParams(-1, dp(50)).apply { bottomMargin = dp(8) })
    ...
    private fun createMap(container: FrameLayout) {
        ...
        map = null
        routeLine = null
        val mapsKey = getString(R.string.maps_key).trim()
        if (mapsKey.isBlank() || mapsKey.startsWith("YOUR_")) {
            container.addView(TextView(this).apply {
                text = "Peta Google Maps belum dikonfigurasi.\nTracking GPS tetap dapat berjalan dan titik lokasi tetap tersimpan."
                gravity = Gravity.CENTER
                textSize = 13f
                setTextColor(color(R.color.nf_text_secondary))
    ...
    private fun requestLocation() {
        ...
        if (need.isNotEmpty()) {
            // Preserve the session details while the runtime permission dialog is open.
            prefs.edit()
                .putString("pending_name", sessionName)
                .putString("pending_type", selectedType)
                .apply()
    ...
    private fun startTracking() {
        sessionName = prefs.getString("pending_name", sessionName)?.ifBlank { "Aktivitas" } ?: "Aktivitas"
        selectedType = prefs.getString("pending_type", selectedType)?.ifBlank { "Running" } ?: "Running"
        tracking = true
    ...
    private val stopReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            ...
            if (id != null) {
                ...
                if (a != null) {
                    // Media is persisted by TrackingService together with the activity.
                    // The Activity may be gone when a background service finishes, so do not
                    // make saving attachments depend on this receiver being alive.
                    prefs.edit().remove("pending_name").remove("pending_type").apply()
                    runCatching { FirebaseSync.syncActivity(this@MainActivity, a, a.media) }
                }
            }
